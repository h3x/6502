...
print_hex:
  ...
  lsr
  lsr
  jsr print_nibble   ; high nibble
  pla
  and #$0F
  jsr print_nibble   ; low nibble
  rts
print_nibble:
  cmp #$0A
  bcc digit
  adc #$06        ; adjust for A-F (since carry is clear)
digit:
  adc #'0'
  jsr print_char
  rts
...
keyboard_interrupt:
  ...
  lda PORTA        ; read raw scancode byte

  pha              ; save copy
  jsr print_hex    ; print hex
  jsr print_space  ; print space
  pla              ; restore (not strictly needed now)
...
keymap:
  ...
  .byte "?,kio09??./l;p-?" ; 40-4F  (FIXED alignment)
  .byte "??'?[=????",$0a,"]?\\??" ; 50-5F  (fixed backslash + enter)
  .byte "?????????1?47???" ; 60-6F
...
keymap_shifted:
  ...
  .byte "?<KIO)(??>?L:P_?" ; 40-4F  (FIXED to match unshifted)
  .byte '??"?{+?????}?|??' ; 50-5F  (correct quotes + backslash → |)
  .byte "?????????1?47???" ; 60-6F
